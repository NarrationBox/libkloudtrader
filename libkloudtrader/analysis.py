...
def money_flow_index(high, low, close, volume, n, fillna=False):
    try:
        mfi_data=ta.momentum.money_flow_index(high, low, close, volume, n, fillna)
        return mfi_data
    except Exception as e:
        raise (e)
...
#Ultimate Oscillator
def ultimate_oscillator(high, low, close, short_period, medium_period, long_period):
    try:
        uo_data=talib.ULTOSC(high, low, close, timeperiod1=short_period, timeperiod2=medium_period, timeperiod3=long_period)
        return uo_data
    except Exception as e:
        raise(e)
...
def chaikin_money_flow(high, low, close, volume, n, fillna=False):
    try:
        cmf_data=ta.volume.chaikin_money_flow(high, low, close, volume, n, fillna=fillna)
        return cmf_data
    except Exception as e:
        raise(e)
...
def donchian_channels(close,n,fillna=False):
    try:
        ...
        dc_low_band_indicator=ta.volatility.donchian_channel_lband_indicator(close,n,fillna)
        df=pd.DataFrame()
...
def ease_of_movement(high, low, close, volume, n, fillna=False):
    try:
        eom_data=ta.volume.ease_of_movement(high, low, close, volume, n, fillna)
        return eom_data
    except Exception as e:
        raise(e)
...
#Ichimoku Cloud
def ichimoku_cloud(high, low, short_period, medium_period, long_period,fillna=False):
    try:
        ichimoku_cloud_a=ta.trend.ichimoku_a(high, low, short_period,medium_period,fillna)
        ichimoku_cloud_b=ta.trend.ichimoku_b(high, low, medium_period,long_period,fillna)
        df=pd.DataFrame()
...
def keltner_channels(high, low, close, n, fillna=False):
    try:
        df=pd.DataFrame()
        df['keltner_channel_central']=ta.volatility.keltner_channel_central(high, low, close, n, fillna)
        df['keltner_channel_highband']=ta.volatility.keltner_channel_hband(high, low, close, n, fillna)
        df['keltner_channel_highband_indicator']=ta.volatility.keltner_channel_hband_indicator(high, low, close, n, fillna)
        df['keltner_channel_lowband']=ta.volatility.keltner_channel_lband(high, low, close, n, fillna)
        df['keltner_channel_lowband_indicator']=ta.volatility.keltner_channel_lband_indicator(high, low, close, n, fillna)
        return df
    except Exception as e:
        raise(e)
...
def know_sure_thing(close,r1,r2,r3,r4,n1,n2,n3,n4,nsig,fillna=False):
    try:
        kst_data=ta.trend.kst_sig(close,r1,r2,r3,r4,n1,n2,n3,n4,nsig,fillna)
        return kst_data
    except Exception as e:
        raise(e)
...
def macd(close, short_period, long_period, n_sign, fillna=False):
    try:
        ...
        df['macd_signal']=ta.trend.macd_signal(close, short_period, long_period,n_sign, fillna)
        df['macd_difference']=ta.trend.macd_diff(close, short_period, long_period,n_sign, fillna)
        return df
    except Exception as e:
        raise(e)
...
def mass_index(high, low, short_period, long_period, fillna=False):
    try:
        mi_data=ta.trend.mass_index(high, low, short_period, long_period, fillna)
        return mi_data
    except Exception as e:
        raise(e)
...
def negative_volume_index(close,volume,fillna=False):
    try:
        nvi_data=ta.volume.negative_volume_index(close, volume, fillna=fillna)
        return nvi_data
    except Exception as e:
        raise(e)
...
#Parabolic SAR
def parabolic_sar(high, low, acceleration=0, maximum=0): #Acceleration and max 0 by default
    try:
...
def rate_of_change(close,n):
    try:
        ...
        df['rate_of_change_ratio_100_scale']=talib.ROCR100(close, timeperiod=n)
        return df
    except Exception as e:
        rasie(e)
...
def vortex_indicator(high,low,close,n,fillna=False):
    try:
        df=pd.DataFrame()
        df['vortex_inidicator_positive']=ta.trend.vortex_indicator_pos(high, low, close, n, fillna)
        df['vortex_indicator_negative']=ta.trend.vortex_indicator_neg(high, low, close, n, fillna)
        return df
    except Exception as e:
        raise(e)
...
def coppock_curve(close,n):
    try:
        ...
        ROC2 = M / N
        Copp = pd.Series((ROC1 + ROC2).ewm(span=n, min_periods=n).mean(), name='coppock_curve')
...
def volume_adjusted_moving_average(close, volume, n):
    try:
        ...
        vama = [sum(close_vol[idx+1-n:idx+1]) / n for idx in range(n-1, len(close))]
...
def annual_sharpe_ratio(close):
    if len(close)<250 or len(close)>255:
        return ("For annual sharpe ratio close data should constitue data only for 1 year. To calculate sharpe ratio for n number of days use sharpe_ratio")
    try:
...
def omega_ratio(close, risk_free=0.0, required_return=0.0, trading_days=252):
    try:
        rets=daily_returns(close)
        omr_data=empyrical.omega_ratio(rets,risk_free=risk_free, required_return=required_return, annualization=trading_days)
        return omr_data
    except Exception as e:
        raise(e)
...
def alpha(close,benchmark_close,risk_free=0.0, period='daily', annualization=None, _beta=None):
    try:
        ...
        alpha_data=empyrical.alpha(rets,benchmark_rets,risk_free=risk_free,period=period,annualization=annualization,_beta=_beta)
        return alpha_data
    except Exception as e:
        raise(e)
...
#Downside Risk
def downside_risk(close,required_return=0, period='daily', annualization=None):
    try:
        rets=daily_returns(close)
        dr_data=empyrical.downside_risk(rets,required_return=required_return,period=period,annualization=annualization)
        return dr_data
    except Exception as e:
        raise(e)
...
def value_at_risk(close,tabular=True):
    try:
        ...
        if tabular==True:
            data=tabulate([["90%",var_90],["95%",var_95],["99%",var_99]],headers=["Confidence Level","Value at Risk"])
            return data
        if tabular==False:
